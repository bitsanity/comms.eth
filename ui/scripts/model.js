...
function ΞloadKey( acct, acctPass ) {
  ...
  let keyObj = global.keythereum.importFromFile( acct );
  if (!keyObj) throw "can't get private key";

  result.privkey = global.keythereum.recover( acctPass, keyObj );
...
function ΞmakeName( acct, acctPass, newname, val, gasprice ) {
  ...
  .then( () => {
    ...
    .send( {from: acct, gas: 100000, gasPrice: gasprice} )
    .catch( err => { console.log(err.toString()) } );
...
function ΞmakeTopic( acct, acctPass, newtop, val, gasprice ) {
  ...
  .then( () => {
    ...
    .send( {from: acct, value: val, gas: 120000, gasPrice: gasprice} )
    .catch( err => { console.log(err.toString()) } );
...
ΞretrieveMessages( blockstogoback, toaddr, fromaddr, keywords, cb ) {
  ...
  for( let ii = fromblock; ii <= blocknum; ii++ ) {
    ...
    for( let jj = 0; jj < blk.transactions.length; jj++ ) {
      if ( !blk.transactions[jj].input || blk.transactions[jj].input == 0 )
        continue;

      if (    usefrom && fromaddr != blk.transactions[jj].from
           || toaddr && toaddr != blk.transactions[jj].to )
        continue;
      
      // this only works on unencrypted messages
...
async function
ΞpostMessage( toaddr, acct, acctPass, val, messagehex, gasprice ) {
  ...
  web3.eth.personal.unlockAccount( acct, acctPass, 100 )
  .then( () => {

    let valopt = (val && val.length != 0) ? val : "0";
...
function ΞaddressToName( hash, addr, cb ) {
  if (addr == '0x8c34F41f1cf2dfe2C28B1Ce7808031c40CE26d38') {
    cb( hash, 'admin.blabb.eth' );
    return;
  }
  else if (addr == '0x147b61187F3F16583AC77060cbc4f711AE6c9349') {
